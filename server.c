...
static int enviar_erro(int client_socket, const char *texto) {
    ...
    return send_all(client_socket, &resposta, sizeof(resposta));
}
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(protocolo, "v4") != 0 && strcmp(protocolo, "v6") != 0) {
        fprintf(stderr, "Erro: protocolo deve ser v4 ou v6.\n");
        mostrar_uso(argv[0]);
        return 1;
    }

    if (porta_valida(argv[2], &porta) != 0) {
        fprintf(stderr, "Erro: porta deve ser um numero entre 1 e 65535.\n");
        mostrar_uso(argv[0]);
        return 1;
    }

    if (palpite_valido(argv[3], senha_secreta) != 0) {
        fprintf(stderr, "Erro: senha deve conter exatamente 5 digitos.\n");
        mostrar_uso(argv[0]);
        return 1;
    }
    ...
    if (send_all(client_socket, &msg, sizeof(msg)) <= 0) {
    ...
    while (1) {
        int recebido = recv_all(client_socket, &msg, sizeof(msg));
        ...
        enviar_feedback(client_socket, &msg, senha_secreta, &contador_tentativas);
        if (msg.type == MSG_WIN) {
            break;
        }
    }
...
void enviar_feedback(int client_socket, HackerMessage *msg, const int senha_secreta[5], int *contador_tentativas) {
    ...
    (*contador_tentativas)++;
    msg->attempts = *contador_tentativas;
    calcular_feedback(senha_secreta, msg->guess, msg->feedback);
    int acertos = 0;
    for (int i = 0; i < 5; i++) {
    ...
    if (send_all(client_socket, msg, sizeof(HackerMessage)) <= 0) {
